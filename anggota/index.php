...
try {
    ...
    // Fetch members
    if (!empty($search)) {
        $query = "SELECT * FROM anggota WHERE nama LIKE :search1 OR nim LIKE :search2 OR email LIKE :search3 ORDER BY nama ASC LIMIT :limit OFFSET :offset";
        ...
        $query = "SELECT * FROM anggota ORDER BY nama ASC LIMIT :limit OFFSET :offset";
        $stmt = $db->prepare($query);
        $stmt->bindValue(':limit', $limit, PDO::PARAM_INT);
        $stmt->bindValue(':offset', $offset, PDO::PARAM_INT);
        $stmt->execute();
